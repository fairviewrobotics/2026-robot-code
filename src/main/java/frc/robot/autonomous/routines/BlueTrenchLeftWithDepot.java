...
public class BlueTrenchLeftWithDepot extends SequentialCommandGroup {
    public BlueTrenchLeftWithDepot(SwerveSubsystem swerveSubsystem, IntakeSubsystem intakeSubsystem, TurretSubsystem turretSubsystem, ShooterSubsystem shooterSubsystem, HoodSubsystem hoodSubsystem, IndexerSubsystem indexerSubsystem) {
        setName("BLUE TRENCH LEFT WITH DEPOT");
        addRequirements(swerveSubsystem, intakeSubsystem, turretSubsystem, shooterSubsystem, hoodSubsystem);
        addCommands(
                Commands.defer(() -> {
                    ...
                    return new SequentialCommandGroup(
                            ...
                            new SequentialCommandGroup(
                                    new DriveToPointContinuous(swerveSubsystem, AllianceFlipUtil.apply(FieldConstants.BLUE_TRENCH_LEFT_TRANSITION_CONTINUOUS), 0.5 * AUTO_SCALAR),
                                    new DriveToPointContinuous(swerveSubsystem, AllianceFlipUtil.apply(FieldConstants.BLUE_TRENCH_LEFT_TRANSITION_CONTINUOUS), 0.5 * AUTO_SCALAR),
                                    ...
                                    new DriveToPoint(swerveSubsystem, AllianceFlipUtil.apply(FieldConstants.BLUE_DEPOT_TRANSITION), 1.0 * AUTO_SCALAR),
                                    new ParallelDeadlineGroup(
                                            new DriveToPoint(swerveSubsystem, AllianceFlipUtil.apply(FieldConstants.BLUE_DEPOT_PICKUP), 1.0 * AUTO_SCALAR),
                                            new IntakeCommand(intakeSubsystem)
                                    ),
                                    new DriveToPoint(swerveSubsystem, AllianceFlipUtil.apply(FieldConstants.BLUE_DEPOT_TRANSITION), 1.0 * AUTO_SCALAR),
                                    new ParallelDeadlineGroup(
                ...
                }, Set.of(swerveSubsystem, intakeSubsystem, turretSubsystem, shooterSubsystem, hoodSubsystem))
        );
    }
}
